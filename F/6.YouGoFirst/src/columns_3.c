...
int main(int argc, char* argv[])
{
	...
	MPI_Datatype column_raw;
	MPI_Type_vector(10, 1, comm_size, MPI_INT, &column_raw);

	// Resize it
	MPI_Datatype column;
	MPI_Type_create_resized(column_raw, 0, sizeof(int), &column);
	MPI_Type_commit(&column);

	// Use it in a scatter
	MPI_Scatter(values, 1, column, my_values, 10, MPI_INT, 0, MPI_COMM_WORLD);
